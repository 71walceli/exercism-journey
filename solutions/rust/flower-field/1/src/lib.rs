...
fn check_garden(garden: &[&str]) -> bool {
    ...
    let width = garden[0].as_bytes().len();
    for row in garden.into_iter().skip(1) {
        if row.len() != width {
            return false;
        }
    }
...
fn _build<'a>(garden: &[&str]) -> Garden {
    ...
    if height > 0 {
        width = garden[0].as_bytes().len();
        grid = Vec::new();
        grid.push(vec![0_u8; width+2]);
        for row in garden.into_iter() {
            let mut new_row = Vec::new();
            new_row.push(0_u8);
            for chr in row.as_bytes().into_iter() {
