...
fn bench_large_parallel(b: &mut Bencher) {
    let texts = all_texts(3000);
    b.iter(|| parallel_letter_frequency::frequency(&texts, 3));
}

#[bench]
fn bench_large_parallel_max_threads(b: &mut Bencher) {
    let texts = all_texts(3000);
    b.iter(|| {
        let worker_count = thread::available_parallelism().unwrap().get();
        println!("Worker count: {}", worker_count);
        parallel_letter_frequency::frequency(&texts, worker_count)
    });
}
