...
pub fn plants(diagram: &str, student: &str) -> Vec<&'static str> {
    ...
    let diagram_width = diagram.chars().position(|c| c == '\n');
    if diagram_width.is_none() {
        panic!("Invalid diagram string, missing newline.");
    }
    let diagram_width = diagram_width.unwrap();
